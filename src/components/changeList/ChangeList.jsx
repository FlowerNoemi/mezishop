    ...
    e.preventDefault();
        try {
            const response = await axios.post(Edit_URL,
                {id:id, termeknev:termeknevmod, mennyiseg:mennyisegmod, ar:armod , status:currency, img:selectedFile} ,
                	{
...
return (
        <div className='backgroundChange'>
				<div>
                	<p className={success ? 'successChangeProduct' : 'offscreenChangeProduct'} aria-live='assertive'>{success}</p>  
            	</div>
				<div>
                	<p ref={errRef} className={errMsg ? 'errmsgChangeProduct' : 'offscreenChangeProduct'} aria-live='assertive'>{errMsg}</p>  
            	</div>
            {open ? ( 
                <Box
                component='form'
                ...
                <div>
                    <TextField
                        sx={{ 
                            ...
                            FormHelperTextProps={{style: {fontFamily:'Arima Madurai, sans-serif' }}}
                            variant='standard'>
				...
				<div className='listBox'>
      				<List dense sx={{ width: '100%', maxWidth: 400, height:100,  marginBottom:'50px'}} className='list'>
            			<ListItem
              				key={id}
