...
if (isset($_POST["email"]) and isset($_POST["userid"]) and isset($_POST["termekid"]) and isset($_POST["termeknev"]) and isset($_POST["db"]) and isset($_POST["ar"]) and isset($_POST["img"])) {
    ...
    if ((!empty($data[0]["status"]) and $data[0]["status"] == "folyamatban")) {
        $stmt1 = $dbo->prepare("SELECT id, termekid, userid,orderid, db FROM rendeles WHERE userid LIKE :userid and termekid LIKE :termekid and orderid LIKE :orderid");
        ...
        try {
            ...
            if ((!empty($data1)) and ($data[0]["status"] == "folyamatban") and ($data1[0]["orderid"] == $orderid and $db == 0)) {
